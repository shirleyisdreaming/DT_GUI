...
# Main function
if len(sys.argv)!=2:
 print("Usage:\ anapy ff_asym_link.py paramFNforDT")
paramFN_link=sys.argv[1]


root = Tk.Tk()
root.wm_title("FF display v0.1 Dt. 2017/03/29 hsharma@anl.gov")
figur = Figure(figsize=(19.5,6.5),dpi=100)
...
refreshPlot = 0
Hydra=0
paramContents_link = open(paramFN_link, 'r').readlines()
for line in paramContents_link:
    if line.startswith('FileStem'):
        fStem_link = line.split()[1]
    if line.startswith('RawFolder'):
        RawFolder = line.split()[1]
    if line.startswith('startNr'):
        firstFileNumber = line.split()[1]
    if line.startswith('Dark'):
        darkfilefullpath = line.split()[1]
    ...
    if line.startswith('Lsd'):
        lsdlocal = float(line.split()[1])
    
    if line.startswith('BytesPerPx'):
        BytesPerPixel = int(line.split()[1])
    ...
    if line.startswith('IsHydra'):
        Hydra = int(line.split()[1])
    if(Hydra==1):
        hydraVar.set(1)
    if line.startswith('BC'):
        bclocal[0] = float(line.split()[1])
        bclocal[1] = NrPixelsZ-float(line.split()[2])
        if(Hydra==1):
           bclocal[1] = float(line.split()[2])
        
        
firstfilefullpath=f'{RawFolder}{fStem_link}'+'_'+f'{str(firstFileNumber).zfill(padding)}'+f'{ext}'
fullfilename = firstfilefullpath.split('/')[-1].split('.')[0]
fileStem = '_'.join(fullfilename.split('_')[:-1])
#firstFileNumber = int(fullfilename.split('_')[-1])
firstFileNrVar.set(firstFileNumber)
NrPixelsYVar.set(str(NrPixelsZ))
NrPixelsZVar.set(str(NrPixelsY))
BytesVar.set(BytesPerPixel)
lsdlocalvar.set(str(lsdlocal))
bclocalvar1.set(str(bclocal[0]))
bclocalvar2.set(str(bclocal[1]))
nDetectors = 1
